...
- **Split release notes** — app update modal shows changelog text only; locale navigation links appear in `docs/changelog.md` and on the GitHub release description (pointing to attached `changelog-pt.md` / `changelog-es.md` assets); the compare link (`Full Changelog`) is GitHub-only
