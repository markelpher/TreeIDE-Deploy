...
- **Abas de visualização de arquivo por projeto** — editar conteúdos de arquivos iniciais antes de construir
...
- **Notas de lançamento divididas** — modal de atualização do aplicativo mostra apenas o texto do changelog; links de navegação de localidade aparecem em `docs/changelog.md` e na descrição do lançamento do GitHub (apontando para os ativos `pt.md` / `es.md` anexados); o link de comparação (`Full Changelog`) é exclusivo do GitHub
...
- **Distribuição** expandida de apenas MSI do Windows (v1) para CI multi-OS com suporte a ARM64, Flatpak e pacotes macOS
...
- **Barra de abas de visualização de arquivos** — abas de arquivos rolam em uma região dedicada ao lado do distintivo de tipo de arquivo e botão de fechar; removido o divisor vertical; altura da barra estável com rolagem por seta apenas (sem expansão de barra de rolagem ao passar o mouse)
- **Tipografia do editor de visualização de arquivos** — tamanho monoespaçado compartilhado com os visualizadores de arquivos do Estúdio de Modelos e Construção
...
- **Editor de estrutura de modelos** — Tab indenta nas áreas de texto de estrutura e arquivo em vez de pular o foco para outros controles; a visualização da árvore é atualizada ao vivo enquanto se digita; a lista de modelos embutidos não deixa mais um espaço abaixo do último item quando o rodapé personalizado está oculto
- **Vínculos de modal de modelos** — ouvintes de editor de estrutura/arquivo se reanexam após a nova renderização da lista para que edições inline continuem funcionando
...
- **Nome da versão de lançamento no diálogo de atualização** — o modelo `Tree IDE v${version}` do electron-builder é normalizado para a string de versão real no aplicativo
...
- **Validação de indentação** — tabs misturados e espaços são detectados e relatados no painel de validação
...
- **Editor de visualização de arquivos** — barra de rolagem e edição por teclado (Tab / Backspace) correspondem ao editor de estrutura principal; peso e tamanho da fonte agora correspondem ao visualizador de arquivos do Templates
- **Nomeação de salvar / exportar** — saves e exports de `.tree` usam o nome do projeto resolvido em vez de um genérico Untitled fallback
- **Persistência de modelos personalizados** — modelos personalizados são salvos automaticamente enquanto o modal de Modelos está aberto e restauram do IndexedDB após reiniciar
...
- **Empacotamento apenas MSI do Windows** como o único formato de distribuição — substituído por NSIS, MSI, portátil e artefatos Linux/macOS
