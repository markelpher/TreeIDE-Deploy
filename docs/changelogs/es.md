...
- **Previsualización por archivo** — al hacer clic en un archivo en la previsualización de la estructura se abre un panel de editor de ancho completo con una insignia de tipo de archivo (mismo diseño de un solo panel para plantillas integradas y personalizadas)
...
- **Windows** — instalador NSIS, MSI y versiones portátiles para x64 y ARM64; instalador multilenguaje (inglés, portugués y español) con título de selector de idioma localizado
...
- **Flujo de construcción** — el botón de la barra de herramientas **Construir** ahora abre **Estudio de Construcción** en lugar de escribir archivos inmediatamente
...
- **Pantalla de plantillas** — el editor de estructura y la previsualización del árbol utilizan una división 50/50 en modo de edición personalizada; la etiqueta de previsualización se acorta a **Previsualización**; la acción **Usar plantilla** centrada en el pie de modal
...
- **Tipografía del editor de previsualización de archivos** — tamaño monoespaciado compartido con los visores de archivos de Plantillas y Estudio de Construcción
...
- **Pantalla en blanco / negra después de la instalación** — las construcciones empaquetadas podrían enviarse sin `dist/renderer/` porque la salida de la UI está ignorada por git; `electron-before-pack` ahora construye y verifica el renderizador antes de cada empaquetado de `electron-builder`
...
- **Nombre de lanzamiento del diálogo de actualización** — la plantilla `Tree IDE v${version}` de electron-builder se normaliza a la cadena de versión real en la aplicación
...
- **Seguridad de ruta** — el analizador y creador de árbol rechazan rutas de travesía y otras rutas inseguras antes de escribir en disco
...
- **Editor de previsualización de archivos** — la barra de desplazamiento y la edición por teclado (Tab / Retroceso) coinciden con el editor de estructura principal; el peso y tamaño de la fuente ahora coinciden con el visor de archivos de Plantillas
- **Nombrado de guardado / exportación** — los guardados y exportaciones `.tree` utilizan el nombre de proyecto resuelto en lugar de un genérico Sin título
...
- **Construcción del instalador NSIS de Windows** — las cadenas en español utilizan LCID **3082** (`SpanishInternational`, coincidiendo con `es_ES` de electron-builder); se eliminaron las sobreescrituras redundantes de `Spanish-1034.nsh` y `MUI_UNTEXT_*` en portugués para que `makensis -WX` ya no falle por cadenas de idioma duplicadas o faltantes
- **Idioma del instalador NSIS de Windows** — selector de idioma en el asistente (inglés, portugués, español) sin reinicio o archivos temporales auxiliares; **Siguiente** avanza inmediatamente y las páginas de directorio, progreso, finalización y personalizadas aplican títulos, descripciones y botones traducidos en el momento de la visualización
- **Idioma del desinstalador NSIS de Windows** — mismo selector de idioma en el asistente; las páginas de progreso y finalización siguen el idioma seleccionado sin reinicio
...
- **Interfaz de construcción de un solo paso** — sustituida por el Estudio de Construcción; la creación directa de carpetas sigue estando disponible dentro del estudio
