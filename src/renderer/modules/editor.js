...
export function createEditor(app) {
    ...
    function getActiveFileHistoryKey(filePath = app.state.activePreviewPath) {
        const tabId = app.tabs.getActiveTab()?.id || 'default';
        return filePath ? `${tabId}\u0000${filePath}` : '';
    }
    ...
    function bindEditorKeys() {
        ...
        document.addEventListener(
            ...
            (e) => {
                ...
                if (!isTreeEditor && !isPreviewEditor && !isTemplateStructureEditor && !isTemplateFileEditor) {
                    return;
                }
    ...
    function insertTabInTextarea(textarea, e) {
        const afterChange =
            textarea.id === 'templateTreeEditor' || textarea.id === 'templateFileEditor'
                ? (el) => {
    ...
    function openFilePreview(filePath) {
        ...
        app.undoredo.prepareFileHistory(getActiveFileHistoryKey(filePath), content, 0, 0);
    ...
    function closeFilePreview() {
        ...
        S.activePreviewPath = '';
        if (S.filePreviewPanel) {
            S.filePreviewPanel.classList.remove('show');
        }
        if (S.filePreviewPanel) {
            S.filePreviewPanel.classList.remove('markdown-file');
        }
        if (S.filePreviewEditor) {
            S.filePreviewEditor.value = '';
            S.filePreviewEditor.style.display = '';
        }
        if (S.filePreviewMode) {
            S.filePreviewMode.textContent = '';
        }
        if (S.markdownPreview) {
            S.markdownPreview.textContent = '';
        }
    ...
    function switchAdjacentTab(prev) {
        ...
        const nextIndex = prev ? (currentIndex - 1 + len) % len : (currentIndex + 1) % len;
    ...
    function bindEditorInput() {
        ...
        editor.addEventListener('beforeinput', () => {
            app.undoredo.captureEditorState(editor.value, editor.selectionStart, editor.selectionEnd);
        });
    }

    function applyFileHistory(direction) {
        const S = app.state;
        const fileEditor = S.filePreviewEditor;
        const filePath = S.activePreviewPath;
        if (!fileEditor || !filePath) {
            return false;
        }
        const historyKey = getActiveFileHistoryKey(filePath);
        const snapshot =
            direction === 'undo'
                ? app.undoredo.performFileUndo(historyKey, fileEditor.value, fileEditor.selectionStart, fileEditor.selectionEnd)
                : app.undoredo.performFileRedo(historyKey, fileEditor.value, fileEditor.selectionStart, fileEditor.selectionEnd);
        if (!snapshot) {
            return false;
        }

        fileEditor.value = snapshot.value;
        fileEditor.setSelectionRange(Math.min(snapshot.cursorStart, snapshot.value.length), Math.min(snapshot.cursorEnd, snapshot.value.length));
        delete app.fileops.getDefaultFileLangs()[filePath];
        S.fileContents[filePath] = snapshot.value;
        updateMarkdownPreview();
        app.fileops.persistFileContents();
        const activeTab = app.tabs.getActiveTab();
        if (activeTab) {
    ...
    function bindPreviewEditor() {
        ...
        if (!filePreviewEditor.dataset.boundEditorKeys) {
            ...
            filePreviewEditor.addEventListener(
                'keydown',
                (e) => {
        ...
        filePreviewEditor.addEventListener('beforeinput', () => {
            if (!S.activePreviewPath) {
                return;
            }
            app.undoredo.captureFileEditorState(
                getActiveFileHistoryKey(),
                filePreviewEditor.value,
                filePreviewEditor.selectionStart,
                filePreviewEditor.selectionEnd
            );
        });
        ...
        filePreviewEditor.addEventListener('input', () => {
            ...
            app.undoredo.pushFileUndoState(
                getActiveFileHistoryKey(),
                filePreviewEditor.value,
                filePreviewEditor.selectionStart,
                filePreviewEditor.selectionEnd
    ...
    return {
        ...
        bindTreeViewClicks,
        bindZoomWheel,
        performFileUndo: () => applyFileHistory('undo'),
        performFileRedo: () => applyFileHistory('redo')
    };
}
